...
{
    public class CoinLPStorage : BaseStorage<CoinLP>, IDisposable
    {
        public const string LOCAL_STORAGE_KEY = "Liquify_CoinLPStore";
        private bool disposedValue;

        ILocalStorageService localStorageService { get; set; }
        public CoinLPStorage(ILocalStorageService localStore, StoragePersistanceOption persistanceOption = StoragePersistanceOption.MemoryOnly) : base(persistanceOption)
        {
            localStorageService = localStore;
            this.OnChange += CoinStorage_OnChange;

            //we are filtering on the created events so it makes
            //no sense to keep pulling the info in, just pull
            //it from the last block.
        ...
        public override async Task<bool> LoadData()
        {
            ...
            Database = JsonSerializer.Deserialize<ConcurrentDictionary<string, CoinLP>>(jsonData);
